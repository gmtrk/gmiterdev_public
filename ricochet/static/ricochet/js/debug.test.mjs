...
test('SLIDER_KEYS includes the new keys and no longer includes paddleE', () => {
  ...
  assert.ok(!SLIDER_KEYS.includes('minPegSpacing'));
  assert.ok(!SLIDER_KEYS.includes('pegPitchX'));
  assert.ok(!SLIDER_KEYS.includes('pegPitchY'));
  assert.ok(!SLIDER_KEYS.includes('pegFieldTop'));
});
